 ...
 * 
 */
public class ReviewToolStatisticsView extends ViewPart implements  ISelectionListener{
	...
	public class ReloadStatisticsAction extends Action {
		...
		@Override
		public void run() {
			statisticsTable.setInput(new StatisticsTableInput(input.getReviewInstance(), input.getSourceSet(),true));
		}
	}
	...
	protected TableViewer createTableViewer(Composite parent) {
		...
		ReviewToolStatisticsTableLabelProvider p = new ReviewToolStatisticsTableLabelProvider();
		for ( String col : p.getColumnNames()){
			TableViewerColumn tc = new TableViewerColumn(viewer, SWT.NONE);
			TableColumn c = tc.getColumn();
			c.setText(col);
			c.setWidth(100);
			c.setMoveable(true);
		}
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		if (!selection.isEmpty() && selection instanceof StructuredSelection){
			...
			if (sourceSetCandidate instanceof ReviewSourceSetView){
				...
				if (riCandidate != null){
					statisticsTable.setInput(new StatisticsTableInput(riCandidate, ((ReviewSourceSetView) sourceSetCandidate).getModelElement(),false));
				}
			} else if (sourceSetCandidate instanceof SourceTreeElement){
				ReviewInstance riCandidate = (ReviewInstance) part.getAdapter(ReviewInstance.class);
				SourceTreeElement ste = (SourceTreeElement) sourceSetCandidate;
				if (riCandidate != null){
					statisticsTable.setInput(new StatisticsTableInput(riCandidate, ste.getSet(),false));
				}
	...
	private void selectReportEntry(SourceTreeElement ste) {
		if (statisticsTable.getInput() instanceof StatisticsTableInput){
			StatisticsTableInput ip = (StatisticsTableInput) statisticsTable.getInput();
			if (ip.getReport() != null){
				for (ReportEntry re : ip.getReport()){
					if (re.getSourceFile() == ste.getSource()){
						statisticsTable.setSelection(new StructuredSelection(re),true);
						break;
					}
				}
			}
	...
	protected void createContextMenus() {
		...
		statisticsTable.getControl().setMenu(viewMenu.createContextMenu(statisticsTable.getControl()));
